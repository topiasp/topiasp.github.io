...
var urlit =[
"http://pxnet2.stat.fi/PXWeb/api/v1/fi/StatFin/tym/tyonv/statfin_pxt_tym_tyonv_001.px",
'http://pxnet2.stat.fi/PXWeb/api/v1/fi/StatFin/vrm/tyokay/010_tyokay_tau_101.px',
'http://pxnet2.stat.fi/PXWeb/api/v1/fi/StatFin/tym/tyonv/statfin_pxt_tym_tyonv_003.px'
]
...
function alustus() {
	...
	/*
	tblObjs=[]
	for (i=0;i<tableMeta.length;i++) {
		//if (pxwebGetMeta(item).variables[0].time) { alert(pxwebGetMeta(item).variables[0].values); }
		
		for (v=0;v<tableMeta[i].variables.length;v++) {
			if (tableMeta[i].variables[v].time==true) {  
				//tbl(id,urli,tUnit,tUnits,tMin) {
				tmpObj=new tbl(id=i,)
			}
		}
...
function haeSarja(muuttuja,aluekoodi,alueNimi) {
				...
				var d1 = [];
				/* -- Pushattava sarja voi olla vain yhtä pitkä kuin pisin valittu sarja 
				var d1 = [];
				
				settienPituudet=[];
				valitutMuuttujat.forEach(function(item){ settienPituudet.push(item.data.length) })
				
				
		
				pisin=Math.max(...settienPituudet);
				if (settienPituudet.length==0) { pisin=sarja.length; }
			
				
				//for (var i = 0; i < sarja.length; i += 1) { 	d1.push([i,sarja[[i]]]); 		}
				// Otetaan viimeisimmästä takaperin maksimisarja
				
				sarja=sarja.slice((sarja.length-pisin),sarja.length)
				*/
